...
func (p *Proxy) handleConnection(clientConn net.Conn, connID uint64) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Printf("[Conn %d] Error reading from client: %v", connID, err)
				}
				return
			}
			if n == 0 {
			...
			if isStartup && len(data) > 8 && data[4] == 0 && data[5] == 3 {
				...
				log.Printf("[Conn %d] Client Startup: user=%s, database=%s", connID, params["user"], params["database"])
				isStartup = false
			} else if data[0] == 'Q' && n > 5 {
				query := string(bytes.Trim(data[5:], "\x00"))
				log.Printf("[Conn %d] Client Query: %s", connID, query)
				...
				if idx < n-1 {
					...
					log.Printf("[Conn %d] Client Prepared Statement: %s", connID, query)
				} else {
					log.Printf("[Conn %d] Client Parse: (malformed, %d bytes)", connID, n)
				}
			} else if data[0] == 'B' && n > 5 {
				params, err := parseBindParameters(data, n)
				if err != nil {
					log.Printf("[Conn %d] Client Bind: failed to parse parameters: %v", connID, err)
				} else {
					log.Printf("[Conn %d] Client Bind Parameters: %v", connID, params)
				}
			} else if data[0] == 'p' {
				log.Printf("[Conn %d] Client Authentication: (password or SASL data)", connID)
			} else if data[0] == 'D' && n > 5 {
				log.Printf("[Conn %d] Client Describe: %s", connID, parseDescribeMessage(data))
			} else if data[0] == 'E' && n == 5 {
				log.Printf("[Conn %d] Client Execute", connID)
			} else if data[0] == 'S' && n == 5 {
				log.Printf("[Conn %d] Client Sync", connID)
			} else if data[0] == 'X' && n == 5 {
				log.Printf("[Conn %d] Client Terminate", connID)
			} else {
				log.Printf("[Conn %d] Client -> PostgreSQL: %x", connID, data)
			}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Printf("[Conn %d] Error reading message type from PostgreSQL: %v", connID, err)
				}
				return
			}
			...
			if _, err := io.ReadFull(reader, lengthBytes); err != nil {
				log.Printf("[Conn %d] Error reading message length: %v", connID, err)
				return
			}
			msgLength := int(binary.BigEndian.Uint32(lengthBytes))
			if msgLength < 4 {
				log.Printf("[Conn %d] Invalid message length: %d", connID, msgLength)
				return
			}
			...
			if _, err := io.ReadFull(reader, body); err != nil {
				log.Printf("[Conn %d] Error reading message body: %v", connID, err)
				return
			}
			...
			case 'R': // Authentication
				if len(body) >= 4 {
					...
					switch authType {
					case 0:
						log.Printf("[Conn %d] PostgreSQL Authentication: Success", connID)
					case 10:
						sasl := string(bytes.Trim(body[4:], "\x00"))
						log.Printf("[Conn %d] PostgreSQL Authentication: SASL requested (%s)", connID, sasl)
					default:
						log.Printf("[Conn %d] PostgreSQL Authentication: Type %d", connID, authType)
					}
				}
			case 'C':
				tag := string(bytes.Trim(body, "\x00"))
				log.Printf("[Conn %d] PostgreSQL Command Complete: %s", connID, tag)
			case 'E':
				fields := parseErrorOrNotice(body)
				log.Printf("[Conn %d] PostgreSQL Error: %s", connID, fields["M"])
			case 'N':
				fields := parseErrorOrNotice(body)
				log.Printf("[Conn %d] PostgreSQL Notice: %s", connID, fields["M"])
			case 'T':
				columns := parseRowDescription(body)
				log.Printf("[Conn %d] PostgreSQL Row Description: %v", connID, columns)
			case 'D':
				values := parseDataRow(body)
				log.Printf("[Conn %d] PostgreSQL Data Row: %v", connID, values)
			case 'Z':
				log.Printf("[Conn %d] PostgreSQL Ready for Query", connID)
			case 'S':
				keyValue := parseParameterStatus(body)
				if len(keyValue) >= 2 {
					log.Printf("[Conn %d] PostgreSQL Parameter Status: %s=%s", connID, keyValue[0], keyValue[1])
				}
			case 'K':
				log.Printf("[Conn %d] PostgreSQL Backend Key Data", connID)
			case '1':
				log.Printf("[Conn %d] PostgreSQL Parse Complete", connID)
			case '2':
				log.Printf("[Conn %d] PostgreSQL Bind Complete", connID)
			default:
				records, err := parsePostgresDataRow(body)
				if err != nil {
					log.Printf("Error parsing hex records: %v", err)
				} else {
					log.Printf("Records: %+v", records)
				}
				log.Printf("[Conn %d] PostgreSQL -> Client: %x", connID, fullMsg)
			}

			// Forward the message to the client
			if _, err := clientConn.Write(fullMsg); err != nil {
				log.Printf("[Conn %d] Error forwarding to client: %v", connID, err)
				return
			}
		}
	}()
